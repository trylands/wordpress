...
puts node['wordpress']['db']['user']
if node.has_key?("ec2")
  server_fqdn = node['ec2']['public_hostname']
else
  server_fqdn = "wordpress"
end
...
execute "mysql-install-wp-privileges" do
  command "/usr/bin/mysql -u root -h \"#{node['wordpress']['db']['host']}\" -p\"#{node['mysql']['server_root_password']}\" < #{node['mysql']['conf_dir']}/wp-grants.sql"
  action :nothing
end
...
execute "create #{node['wordpress']['db']['database']} database" do
  command "/usr/bin/mysqladmin -u root -h \"#{node['wordpress']['db']['host']}\" -p\"#{node['mysql']['server_root_password']}\" create #{node['wordpress']['db']['database']}"
  not_if do
